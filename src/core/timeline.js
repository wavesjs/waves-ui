...
class Timeline extends events.EventEmitter {
  ...
  constructor(pixelsPerSecond = 100, visibleWidth = 1000, {
    ...
    this._trackById = {};
    this._groupedLayers = {};
    this._$elInteractionsMap = new Map();
  ...
  createInteraction(ctor, $el, options = {}) {
    ...
    if (!this._$elInteractionsMap.has($el))
      this._$elInteractionsMap.set($el, new Set());

    const interactionSet = this._$elInteractionsMap.get($el);
  ...
  getHitLayers(e) {
    ...
    this.layers.forEach((layer) => {
      ...
      const boundingRect = layer.$el.getBoundingClientRect();
   ...
   */
  _handleEvent(e) {
    const hitLayers = (e.source === 'surface') ? this.getHitLayers(e) : null;
  ...
  remove(track) {
    ...
    const $el = track.$el;
    const interactions = this._$elInteractionsMap.get($el);
